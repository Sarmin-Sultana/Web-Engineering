...
<?php
    ...
    $conn = new mysqli($host, $dbusername, $dbpassword,  $dbname);
     $sql= "SELECT  description2,teacher_notice FROM `notice` WHERE teacher_notice IS NOT NULL ORDER BY teacher_notice DESC ";
 ...
 <div id="Officer" class="tabcontent">
  <h3>Officer's Job</h3>
   <?php
    ...
    $dbname = "jobportal";
    $conn = new mysqli($host, $dbusername, $dbpassword,  $dbname);
     $sql= "SELECT description1,officer_notice  FROM `notice` WHERE officer_notice IS NOT NULL ORDER BY officer_notice DESC ";
     $result = $conn->query($sql);
        ?>
   <?php foreach($result AS $row ){?>                   
