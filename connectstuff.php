...
$trainning_details=filter_input(INPUT_POST,'trainning_details');
...
if(mysqli_connect_error()){
     ...
     $sql ="INSERT INTO s_school_info(designation,circular_no,circular_date)VALUES('$pname','$bno','$bdate')";
    $result = mysqli_multi_query($conn, $sql);
       
   $sql="INSERT INTO s_personal_info(id,name_bangla,name_english,father_husband_name,mother_name,birth_date,birth_area,present_age,status,nationality,NID,religion)VALUES(LAST_INSERT_ID(),'$name1','$name2','$fname','$mname','$birth','$barea','$page','$status','$nationality','$nid','$religion')";
    $result = mysqli_multi_query($conn, $sql);
        
        
     $sql="INSERT INTO s_parmanent_add(id,village,post_office,telegram,upozilla,district)VALUES(LAST_INSERT_ID(),'$village','$post','$teleg','$upo','$dist')";
        $result = mysqli_multi_query($conn, $sql);
           
        
    $sql="INSERT INTO s_present_address(id,full_address,post_office,telephone,mobile,e_mail)
    VALUES(LAST_INSERT_ID(),'$full','$posto','$phone','$mob','$email')";
        $result = mysqli_multi_query($conn, $sql);  
        
        
         $sql="INSERT INTO
         s_ssc_info(id,school_name,year_start,year_end,exam_name,passed_year,board,result,gpa,subject)
VALUES(LAST_INSERT_ID(),'$school_name','$year_start','$year_end','$exam_name','$passed_year','$board','$result','$gpa','$subject')";
        $result = mysqli_multi_query($conn, $sql);  
        
        
          $sql="INSERT INTO
         s_hsc_info(id,collage_name,year_start,year_end,exam_name,passed_year,board,result,gpa,subject)
VALUES(LAST_INSERT_ID(),'$hscschool_name','$hscyear_start','$hscyear_end','$hscexam_name','$hscpassed_year','$hscboard','$hscresult','$hscgpa','$hscsubject')";
        ...
        $sql="INSERT INTO
         s_honours_info(id,university_name,year_start,year_end,exam_name,result,gpa,passed_year,university,subject)
VALUES(LAST_INSERT_ID(),'$hschool_name','$hyear_start','$hyear_end','$hexam_name','$hresult','$hgpa','$hpassed_year','$hboard','$hsubject')";
        ...
        $sql="INSERT INTO
         s_masters_info(id,university_name,year_start,year_end,exam_name,result,gpa,passed_year,university,subject)
VALUES(LAST_INSERT_ID(),'$mschool_name','$myear_start','$myear_end','$mexam_name','$mresult','$mgpa','$mpassed_year','$mboard','$msubject')";
        $result = mysqli_multi_query($conn, $sql); 
         ...
         $sql="INSERT INTO
s_other_info(id,previous_job,present_job,job_experience,job_agreement,reference,suspention_details,trainning_details,other_info)
VALUES(LAST_INSERT_ID(),'$previous_job','$present_job','$job_experience','$job_agreement','$reference','$suspention_details','$trainning_details','$other_info')";
    $result = mysqli_multi_query($conn, $sql);
        
        
        
 $sql="INSERT INTO
s_bank_info(id,pay_order_no,date,amount)
...
$result = mysqli_multi_query($conn, $sql);
 ...
 $sql="INSERT INTO
 s_image_sign_date(id,image,	signature,date)
